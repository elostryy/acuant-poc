...
function postFrontImage(instanceID, file) {
  return request({
    url:
      '/AssureIDService/Document/' +
      instanceID +
      '/Image?side=0&light=0&metrics=true',
    method: 'POST',
    data: file,
  });
}
...
function postBackImage(instanceID, file) {
  return request({
    url:
      '/AssureIDService/Document/' +
      instanceID +
      '/Image?side=1&light=0&metrics=true',
    method: 'POST',
    data: file,
  });
}
...
function getSignatureImage(instanceID) {
  return request({
    url:
      '/AssureIDService/Document/' + instanceID + '/Field/Image?key=Signature',
    method: 'GET',
    responseType: 'arraybuffer',
  });
}
